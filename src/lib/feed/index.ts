...
export function generateNewsFeed(
  ...
  for (const participant of participants) {
    const { winStreak, loseStreak } = computeStreaks(participant.id, allPredictions, events);
    if (winStreak >= STREAK_THRESHOLD) {
      ...
      feed.push({ id: `wstreak-${participant.id}`, type: "winning_streak", emoji: "\u{1F525}", headline, subtext, playerName: participant.name, playerId: participant.id, priority: 7 });
    }
    if (loseStreak >= STREAK_THRESHOLD) {
      ...
      feed.push({ id: `lstreak-${participant.id}`, type: "losing_streak", emoji: "\u{1F480}", headline, subtext, playerName: participant.name, playerId: participant.id, priority: 7 });
    }
  }
  ...
  for (const outlier of outliers.slice(0, MAX_OUTLIERS)) {
    ...
    const { headline, subtext } = t(outlier.participant.name, outlier.prediction.prediction, outlier.event.event_name, outlier.popularPick);
    feed.push({ id: `outlier-${uid}`, type: "outlier_alert", emoji: "\u{1F52E}", headline, subtext, playerName: outlier.participant.name, playerId: outlier.participant.id, eventId: outlier.event.id, eventName: outlier.event.event_name, sport: outlier.event.sport, priority: 5 });
  }
  if (leaderboard.length >= 2) {
    ...
    if (gap <= 3 && gap >= 0 && first.total_points > 0) {
      ...
      feed.push({ id: `close-race`, type: "close_race", emoji: "\u{26A1}", headline, subtext, priority: 8 });
    }
  }
  if (completedEvents.length > 0 && leaderboard.length >= 2) {
    ...
    for (const e of completedEvents) {
      if (e.decided_at && (!latestDecided || e.decided_at > latestDecided)) latestDecided = e.decided_at;
    }
    const lt = hashPick(LEADER_BANTER_TEMPLATES, `leader-${leader.id}`);
    const { headline: lh, subtext: ls } = lt(leader.name);
    feed.push({ id: `leader-banter`, type: "leader_banter", emoji: "\u{1F451}", headline: lh, subtext: ls, playerName: leader.name, playerId: leader.id, timestamp: latestDecided, createdAt: latestDecided, priority: 8 });
    ...
    feed.push({ id: `last-place-banter`, type: "last_place_banter", emoji: "\u{1F4B8}", headline: bh, subtext: bs, playerName: lastPlace.name, playerId: lastPlace.id, timestamp: latestDecided, createdAt: latestDecided, priority: 8 });
  }
  if (completedEvents.length > 0 && leaderboard.length >= 6) {
    ...
    if (dangerEntries.length > 0) {
      ...
      if (contribution && contribution.contribution >= 40) {
        ...
        feed.push({ id: `lunch-liability-${worst.id}`, type: "lunch_liability", emoji: "\u{1F4B3}", headline, subtext, playerName: worst.name, playerId: worst.id, priority: 6 });
      }
    }
  }
  if (completedEvents.length >= 2 && leaderboard.length >= 3) {
    ...
    if (leaderGap > 0 && leaderGap <= pointsValue) {
      ...
      feed.push({ id: `new-leader`, type: "new_leader", emoji: "\u{1F4AA}", headline, subtext, playerName: leader.name, playerId: leader.id, priority: 9 });
    }
    ...
    if (spudGap > 0 && spudGap <= pointsValue) {
      ...
      feed.push({ id: `new-spud`, type: "new_spud", emoji: "\u{1F954}", headline, subtext, playerName: lastPlace.name, playerId: lastPlace.id, priority: 9 });
    }
  }
  if (leaderboard.length >= 4 && completedEvents.length >= 3) {
    const withAccuracy = leaderboard.filter((e) => e.total_predictions >= 3).map((e) => ({ ...e, pct: Math.round((e.correct_predictions / e.total_predictions) * 100) }));
    if (withAccuracy.length >= 2) {
      ...
      if (best.pct > 0) {
        ...
        feed.push({ id: `accuracy-best`, type: "accuracy_check", emoji: "\u{1F4C8}", headline, subtext, playerName: best.name, playerId: best.id, priority: 6 });
      }
      ...
      if (worst.id !== best.id) {
        ...
        feed.push({ id: `accuracy-worst`, type: "accuracy_check", emoji: "\u{1F4C9}", headline, subtext, playerName: worst.name, playerId: worst.id, priority: 6 });
      }
    }
  }
  ...
  if (participants.length >= 3) {
    ...
    for (const event of upcomingEvents) {
      ...
      if (ratio < 0.5 && pickCount >= 1) {
        ...
        feed.push({ id: `picks-open-${event.id}`, type: "picks_open", emoji: "\u{1F514}", headline, subtext, eventId: event.id, eventName: event.event_name, sport: event.sport, timestamp: bestFeedTimestamp({ decided_at: event.decided_at, event_end_date: event.event_end_date ?? event.close_date, event_date: event.event_date }), priority: 8, odds: event.favourite && event.favourite_odds ? { favourite: event.favourite, favouriteOdds: event.favourite_odds, underdog: event.underdog ?? undefined, underdogOdds: event.underdog_odds ?? undefined } : undefined });
      }
    }
  }
  feed.sort((a, b) => { const cmp = compareFeedRecency(a.timestamp, b.timestamp); if (cmp !== 0) return cmp; return b.priority - a.priority; });
  return feed;
}
