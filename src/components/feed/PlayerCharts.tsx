...
/**
 * Two lightweight, dependency-free SVG charts:
 *   1) Cumulative points over time (sorted by event_id as a recency proxy)
 *   2) Sport mix — horizontal bars showing pick count + win rate per sport
 *
 * Everything uses inline SVG — zero runtime dependency on recharts.
 */
    ...
    >
      <div className="rounded-2xl border border-zinc-200/60 bg-white p-4 shadow-sm">
        ...
        {cumulative.length >= 2 ? (
          ...
          >
            <defs>
              <linearGradient id="bosf-area" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#22c55e" stopOpacity="0.3" />
                <stop offset="100%" stopColor="#22c55e" stopOpacity="0" />
              </linearGradient>
            </defs>
            {areaPath && <path d={areaPath} fill="url(#bosf-area)" />}
            {path && (
              <path
                d={path}
                fill="none"
